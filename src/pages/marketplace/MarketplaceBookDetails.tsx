...
import { Phone, MessageSquare, MapPin, Clock, Share2, Tag, Book, User, ChevronLeft, ChevronRight, Eye } from 'lucide-react';
...
export default function MarketplaceBookDetails() {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-[#0a0a0a]"><div className="w-8 h-8 rounded-full border-4 border-[#00c48c] border-t-transparent animate-spin" /></div>;
  }
  if (!book) return null;

  return (
    <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-[#0a0a0a] pb-24">
      {/* Header */}
      <div className="sticky top-0 z-40 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md border-b border-black/5 dark:border-white/5 px-4 h-14 flex items-center justify-between">
          ...
          <div className="bg-white dark:bg-[#1a1a1a] rounded-[24px] p-5 sm:p-8 border border-black/5 dark:border-white/5 shadow-sm space-y-6">
            <div className="space-y-4">
              <div className="flex items-start justify-between">
                ...
                <div className="text-2xl sm:text-3xl font-black text-[#00c48c] whitespace-nowrap ml-4">৳{book.price}</div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 dark:bg-white/5 rounded-full text-[10px] sm:text-xs font-bold text-gray-600 dark:text-gray-400 border border-black/5">
                  <MapPin size={14} className="text-[#00c48c]" /> {book.district}, {book.upazila}
                </div>
                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 dark:bg-white/5 rounded-full text-[10px] sm:text-xs font-bold text-gray-600 dark:text-gray-400 border border-black/5">
            ...
            <div className="flex items-center gap-4 pt-4 border-t border-black/5 dark:border-white/5">
              <div className="w-12 h-12 rounded-full bg-gray-100 dark:bg-white/5 flex items-center justify-center text-[#00c48c]">
                <User size={24} />
              </div>
              <div className="flex-1">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Seller</p>
                <h4 className="text-sm font-black text-[var(--text)]">{book.seller_name}</h4>
              </div>
              <button 
                onClick={() => navigate(`/messages?userId=${book.user_id}`)}
                className="px-4 py-2 bg-gray-100 dark:bg-white/5 hover:bg-[#00c48c]/10 hover:text-[#00c48c] rounded-xl text-xs font-bold transition-all"
              >
                In-App SMS
              </button>
            </div>
      ...
      {/* Action Footer */}
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-md border-t border-black/5 dark:border-white/5 p-4 z-50">
        <div className="max-w-4xl mx-auto grid grid-cols-3 gap-3">
          <a 
            href={`tel:${book.phone}`}
            className="flex items-center justify-center gap-2 py-3.5 bg-[#00c48c] text-white rounded-2xl font-black text-sm shadow-lg shadow-[#00c48c]/20 hover:scale-[1.02] active:scale-95 transition-all text-center"
          >
            <Phone size={20} /> Call
          </a>
          <button 
            onClick={() => navigate(`/messages?userId=${book.user_id}`)}
            className="flex items-center justify-center gap-2 py-3.5 border-2 border-black/5 dark:border-white/10 text-[var(--text)] rounded-2xl font-black text-sm hover:bg-black/5 dark:hover:bg-white/5 transition-all"
          >
